...
3. Remove manual `Connection` header management—`HttpClient` handles keep-alive automatically and rejects that header.
...
- `WebRequestor.Response` now exposes `getHeaders()` directly and lazily builds `DebugHeaderInfo` on demand. Consumers should register a `setDebugHeaderInfoConsumer` or `setResponseHeaderConsumer` on the Facebook client to observe metadata. The legacy `getLastDebugHeaderInfo`/`getLastResponseHeaders` accessors are deprecated and will be removed in a future release.
